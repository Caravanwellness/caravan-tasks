...
for idx, row in enumerate(english_sheet.iter_rows(values_only=False)):
    ...
    for col_idx, cell in enumerate(row):
        if cell.value and isinstance(cell.value, str):
            ...
            if 'app.allinonewellbeing.com' in cell.value or 'caravanwellness.com' in cell.value or 'caravanwellness.vhx' in cell.value:
                base_link = cell.value.strip()
                match = re.search(r"(?<=\/)[^\/]*$", str(base_link))
                video_page_link = "https://app.allinonewellbeing.com/videos/" + match.group(0)
            # Find the vimeo link
    ...
    for row in csv_reader:
        ...
        if video_page_link in excel_links:
            ...
            if description_col_idx is not None and description:
                ...
                internal_url_cell = english_sheet.cell(row=excel_row_number, column=21)  # Column U is index 21
                internal_url_cell.value = video_link
